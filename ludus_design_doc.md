...
- **Built so far:** Training Ground (faster development), Scouting Network (less fog), Armoury (defensive equipment), Weaponsmith (offensive equipment), Stadium (gate-receipt income for home fixtures) — all match-time, weekly-roll, or result-settlement bonuses on top of existing systems, no new subsystem required.
