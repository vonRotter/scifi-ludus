...
- **Built so far:** Training Ground (faster development), Scouting Network (less fog), Armoury (defensive equipment), Weaponsmith (offensive equipment), Housing (mental-stat boost + larger roster-size cap), Stadium (gate-receipt income for home fixtures) — all match-time, weekly-roll, result-settlement, or roster-cap effects on top of existing systems, no new subsystem required.
